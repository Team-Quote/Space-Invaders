...
// const uint8_t SmallEnemy30PointB[] = { /* bitmap */ };
// const uint8_t SmallEnemy30PointC[] = { /* bitmap */ };
// const uint8_t PlayerShip[]         = { /* bitmap */ };
// const uint8_t BulletSprite[]       = { /* bitmap */ };
// const uint8_t ExplosionSprite[]    = { /* bitmap */ };

void Sprites_SetEnemyPosture(Sprite_t *sprite, EnemyType_t type, EnemyPosture_t posture){
  if(sprite == 0) return;

  if(posture == POSTURE_CLOSE){
    sprite->image = SmallEnemyPointA[type];
  }else{
    sprite->image = SmallEnemyPointB[type];
  }
}
...
void Sprites_InitEnemy(Sprite_t *sprite, uint8_t x, uint8_t y, EnemyType_t type, EnemyPosture_t posture){
  ...
  sprite->y = y;
  sprite->life = true;
  Sprites_SetEnemyPosture(sprite, type, posture);
}
